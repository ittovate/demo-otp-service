...
public class OtpController {
    ...
    @Operation(
            ...
            description= "Generates and sends a One-Time Password (OTP) to the phone number" +
                         "specified in the request. This OTP is stored in order to be verified later on.",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(required = true,description = "Must be a valid phone number"),
            responses = {
                    @io.swagger.v3.oas.annotations.responses.ApiResponse(
                            ...
                            content = @Content(
                                    ...
                                    examples = @ExampleObject(value = "{\"body\": null, \"statusCode\": 201, \"message\": \"Otp sent successfully!\"}")
                    ...
                    @io.swagger.v3.oas.annotations.responses.ApiResponse(
                            ...
                            content = @Content(
                                    schema = @Schema(implementation = ApiResponse.class),
                                    examples = @ExampleObject(value = "{\"body\": null, \"statusCode\": 400, \"message\": \"Validation Error\"}")
                            )
                    ),
                    @io.swagger.v3.oas.annotations.responses.ApiResponse(
                            responseCode = "500",
                            ...
                            content = @Content(
                                    schema = @Schema(implementation = ApiResponse.class),
                                    examples = @ExampleObject(value = "{\"body\": null, \"statusCode\": 500, \"message\": \"Internal server error\"}")
    ...
    @Operation(
            ...
            description = " This endpoint checks the validity of the OTP and confirms the user's" +
                    " identity as well as the token expiry date.",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(required = true,description = "Phone number must be a valid phone number . Token must be a valid numerical 6-digit phone number"),
            responses = {
                    @io.swagger.v3.oas.annotations.responses.ApiResponse(
                            responseCode = "200",
                            ...
                            content = @Content(
                                    schema = @Schema(implementation = ApiResponse.class),
                                    examples = @ExampleObject(value = "{\"body\": null, \"status\": 200, \"message\": \"Token verified successfully!\"}")
                            )
                    ),
                    @io.swagger.v3.oas.annotations.responses.ApiResponse(
                            responseCode = "400",
